// <auto-generated />
using System;
using ActivosFijos.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace ActivosFijos.Data.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20230318163026_Validations")]
    partial class Validations
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.3")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("ActivosFijos.Model.Entities.ActivoFijo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("AnioDepreciacion")
                        .HasColumnType("int");

                    b.Property<int>("DepartamentoId")
                        .HasColumnType("int");

                    b.Property<double>("DepreciacionAcumulada")
                        .HasColumnType("float");

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<DateTime?>("FechaRegistro")
                        .HasColumnType("datetime2");

                    b.Property<int>("TipoActivoId")
                        .HasColumnType("int");

                    b.Property<double>("ValorCompra")
                        .HasColumnType("float");

                    b.Property<double>("ValorDepreciacion")
                        .HasColumnType("float");

                    b.HasKey("Id");

                    b.HasIndex("DepartamentoId");

                    b.HasIndex("TipoActivoId");

                    b.ToTable("ActivosFijo");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            AnioDepreciacion = 2025,
                            DepartamentoId = 1,
                            DepreciacionAcumulada = 0.0,
                            Descripcion = "Laptop",
                            FechaRegistro = new DateTime(2023, 3, 18, 12, 30, 25, 909, DateTimeKind.Local).AddTicks(2347),
                            TipoActivoId = 1,
                            ValorCompra = 25000.0,
                            ValorDepreciacion = 4000.0
                        });
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.AsientosContables", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("CuentaContable")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Descripcion")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.Property<DateTime?>("FechaAsiento")
                        .HasColumnType("datetime2");

                    b.Property<string>("Inventario")
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("MontoAsiento")
                        .HasColumnType("float");

                    b.Property<int>("TipoMovimiento")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("AsientosContables");
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.CalculoDepreciacion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("ActivoFijoId")
                        .HasColumnType("int");

                    b.Property<int?>("AsientoContableId")
                        .HasColumnType("int");

                    b.Property<int>("AñoProceso")
                        .HasColumnType("int");

                    b.Property<string>("CuentaCompra")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CuentaDepreciacion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("DepreciacionAcumulada")
                        .HasColumnType("float");

                    b.Property<DateTime?>("FechaProceso")
                        .HasColumnType("datetime2");

                    b.Property<int>("MesProceso")
                        .HasColumnType("int");

                    b.Property<double>("MontoDepreciado")
                        .HasColumnType("float");

                    b.HasKey("Id");

                    b.HasIndex("ActivoFijoId");

                    b.ToTable("CalculoDepreciacion");
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.Departamento", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Departamento");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Descripcion = "Administracion",
                            Estado = 0
                        },
                        new
                        {
                            Id = 2,
                            Descripcion = "Contabilidad",
                            Estado = 0
                        },
                        new
                        {
                            Id = 3,
                            Descripcion = "Informatica",
                            Estado = 0
                        });
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.Empleado", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("Cedula")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<int>("DepartamentoId")
                        .HasColumnType("int");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.Property<DateTime?>("FechaIngreso")
                        .HasColumnType("datetime2");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("TipoPersona")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DepartamentoId");

                    b.ToTable("Empleado");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Apellido = "Gomez",
                            Cedula = "40213108481",
                            DepartamentoId = 1,
                            Estado = 0,
                            FechaIngreso = new DateTime(2023, 3, 18, 12, 30, 25, 909, DateTimeKind.Local).AddTicks(2308),
                            Nombre = "Carlos",
                            TipoPersona = 0
                        });
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.TipoActivo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("CuentaContableCompra")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CuentaContableDepreciacion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("TipoActivo");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CuentaContableCompra = "65",
                            CuentaContableDepreciacion = "66",
                            Descripcion = "Electronico",
                            Estado = 0
                        });
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Password")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Username")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("User");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Password = "admin1234",
                            Username = "admin"
                        });
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.ActivoFijo", b =>
                {
                    b.HasOne("ActivosFijos.Model.Entities.Departamento", "Departamento")
                        .WithMany()
                        .HasForeignKey("DepartamentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ActivosFijos.Model.Entities.TipoActivo", "TipoActivo")
                        .WithMany("ActivosFijos")
                        .HasForeignKey("TipoActivoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Departamento");

                    b.Navigation("TipoActivo");
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.CalculoDepreciacion", b =>
                {
                    b.HasOne("ActivosFijos.Model.Entities.ActivoFijo", "ActivosFijos")
                        .WithMany()
                        .HasForeignKey("ActivoFijoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ActivosFijos");
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.Empleado", b =>
                {
                    b.HasOne("ActivosFijos.Model.Entities.Departamento", "Departamento")
                        .WithMany("Empleados")
                        .HasForeignKey("DepartamentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Departamento");
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.Departamento", b =>
                {
                    b.Navigation("Empleados");
                });

            modelBuilder.Entity("ActivosFijos.Model.Entities.TipoActivo", b =>
                {
                    b.Navigation("ActivosFijos");
                });
#pragma warning restore 612, 618
        }
    }
}
